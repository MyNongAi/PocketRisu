export const languageKorean = {
    ...
    "help": {
        ...
        "summarizationPrompt": "요약에 사용되는 프롬프트입니다. 비어있으면 기본 프롬프트를 사용합니다. 채팅 데이터에 대해 {{slot}}을 사용하여 ChatML 형식을 사용할 수도 있습니다. HypaV3에서는 요약 결과가 빈 줄(\\n\\n)을 기준으로 청크 단위로 분리되어 유사도 검색에 사용됩니다.",
        ...
        "reSummarizationPrompt": "기존 요약을 다시 요약할 때 사용되는 프롬프트입니다. 비어있으면 기본 프롬프트를 사용합니다. 새로운 채팅의 첫 요약에 사용되는 요약 프롬프트와는 다릅니다.\n\n요약 프롬프트와 마찬가지로 출력이 빈 줄(\\n\\n)을 기준으로 청크 단위로 분리되어 유사도 검색에 사용됩니다.",
        "hypaV3MemoryTokensRatio": "전체 컨텍스트 토큰 중 메모리(요약)에 할당할 비율입니다. 예를 들어 0.2면 컨텍스트 윈도우의 20%를 메모리 요약에 할당합니다.",
        "hypaV3ExtraSummarizationRatio": "요약을 트리거하기 전 목표 토큰 수준을 추가로 낮추는 비율입니다. 예를 들어 0.1이면 최대 컨텍스트의 90%까지 토큰이 줄어들 때까지 요약을 계속하여 여유 공간을 확보합니다.",
        "hypaV3MaxChatsPerSummary": "한 번의 요약 요청에 포함할 최대 채팅 메시지 수입니다. 요약 생성 시 한 번에 묶을 메시지 수를 제한합니다.",
        "hypaV3RecentMemoryRatio": "메모리 토큰 중 최근 요약에 할당할 비율입니다. 최근 요약은 역순(최신 먼저)으로 선택됩니다. 최근 + 유사 비율의 합은 1을 초과할 수 없습니다.",
        "hypaV3SimilarMemoryRatio": "메모리 토큰 중 유사도 기반 요약에 할당할 비율입니다. 현재 대화와의 임베딩 유사도로 선택됩니다. 최근 + 유사 비율의 합은 1을 초과할 수 없습니다.",
        "hypaV3RandomMemoryRatio": "최근과 유사 할당 후 남은 메모리 토큰 비율입니다. 이 부분은 중요 표시된 요약 등에 사용됩니다. 이 값은 자동으로 계산됩니다.",
        "hypaV3PreserveOrphanedMemory": "활성화하면 삭제된 채팅 메시지를 참조하는 요약이 보존됩니다. 비활성화하면 원본 메시지가 없어진 고아 요약이 자동으로 정리됩니다.",
        "hypaV3ProcessRegexScript": "활성화하면 HypaV3 모달에서 요약을 재생성할 때 캐릭터의 정규식 스크립트가 출력에 적용됩니다.",
        "hypaV3DoNotSummarizeUserMessage": "활성화하면 유저 메시지가 요약에서 제외됩니다. 어시스턴트 및 시스템 메시지만 요약됩니다.",
