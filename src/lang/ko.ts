...
export const languageKorean = {
  ...
  errors: {
    ...
    chatBodyMissing: "서버에서 이 채팅 내용을 찾지 못했습니다. 복구 가능한 메시지를 빈 채팅으로 덮지 않도록 원래 상태를 유지했습니다. 다른 기기나 백업을 확인하세요.",
    chatBodyRetry: "채팅 다시 불러오기",
  ...
  importProgress: {
    ...
    failed: "실패",
    readingFile: "파일 읽는 중",
    readingCharx: "CharX 읽는 중",
    readingPng: "PNG 읽는 중",
    savingAssets: "에셋 저장 중",
    savingEmbeddedAssets: "내장 에셋 저장 중",
    savingProfileImage: "대표 이미지 저장 중",
    savingEmotions: "감정 이미지 저장 중",
    savingAdditionalAssets: "추가 에셋 저장 중",
    savingVoiceAssets: "음성 에셋 저장 중",
    savingCardAssets: "카드 에셋 저장 중",
    addingCharacter: "캐릭터 목록에 추가 중",
    readingPreset: "프리셋 읽는 중",
    savingPreset: "프리셋 저장 중",
    readingModule: "모듈 읽는 중",
    savingModule: "모듈 저장 중",
  },
  ...
  stealthUnknown: "메타데이터를 확인하지 못했습니다",
  stealthChecking: "메타데이터 확인 중...",
  protonInspecting: "프로톤 링크 확인 중...",
  protonDownloading: "받는 중",
  protonImportAll: "전부 임포트",
  protonBrowserTitle: "프로톤 폴더",
  protonBrowserSelectAll: "이 폴더 전체 선택",
  protonBrowserSelected: (n: number) => `${n}개 선택됨`,
  protonBrowserImport: (n: number) => n > 0 ? `가져오기 (${n})` : "가져오기",
  protonBrowserLoading: "불러오는 중...",
  protonBrowserRetry: "다시 시도",
  protonBrowserEmpty: "비어 있는 폴더입니다",
  protonBrowserUnsupported: "가져올 수 없는 형식",
  protonBrowserKind: {
    character: "캐릭터",
    module: "모듈",
    preset: "프리셋",
  ...
  menu: "메뉴",
  reloadApp: "새로고침",
  reloadWhileGenerating: "답변을 생성하는 중입니다. 새로고침하면 생성이 멈춥니다. 계속할까요?",
  reloadBlockedUnsaved: "아직 저장되지 않은 변경이 있어 새로고침하지 않았습니다. 저장이 끝난 뒤 다시 눌러주세요.",
  jsonSchema: "JSON 스키마",
  ...
  yumiImportConfirm: "Yumi Provider Manager의 Vertex 모델을 모델 프리셋으로 가져옵니다.\n\n• 모델, 리전, 추론 강도, Flex/Priority, 스트리밍 방식을 그대로 옮기고 서비스 계정 인증 정보도 함께 복사합니다.\n• 지금 파라미터 설정의 온도·top-p·최대 응답 길이와, Yumi처럼 꺼둔 안전 필터도 프리셋에 넣습니다. 이미 가져온 프리셋에는 빠진 값만 채웁니다.\n• 지금 쓰는 모델에 맞는 프리셋을 채팅 봇 모델·보조 모델로 선택합니다. 모델 프리셋으로 보내면 폰을 잠그거나 브라우저를 닫아도 응답이 서버에서 끝까지 생성됩니다.\n• 모델 선택창의 '모델 프리셋' 탭에 프리셋이 생기고, 언제든 기본 모델·플러그인과 바꿔 고를 수 있습니다.\n\n계속할까요?",
  yumiImportNothing: "가져올 Vertex 모델을 찾지 못했습니다.",
  yumiImportDone: (created: number, main: string, updated = 0) => [
    created > 0 ? `모델 프리셋 ${created}개를 만들었습니다.` : '',
    updated > 0 ? `이전에 가져온 프리셋 ${updated}개에 빠진 생성 설정(온도·top-p·최대 응답·안전 필터)을 채웠습니다.` : '',
    main ? `모델로 '${main}'을(를) 선택했습니다.` : '',
  ].filter(Boolean).join(' '),
  modelModeLockLegacy: "모델 선택창 사용 (기본)",
  modelModeLockLegacyDesc: "채팅 봇 탭과 사이드바의 모델 선택창에서 고른 모델을 씁니다. 기본 모델·플러그인·모델 프리셋을 모두 여기서 고를 수 있습니다",
  modelModeLockPreset: "채팅별 프리셋 바인딩 고정",
