...
describe('resolveNamesLocally priority (mirror of the server suite)', () => {
    ...
    it('keeps large manifests indexed instead of broadly fuzzy-scanning every item', () => {
        const items: AssetManifestTuple[] = Array.from({ length: 2_001 }, (_, index) => [
            `bulk-${index}`,
            `assets/bulk-${index}`,
            'png',
        ])
        items.push(['hero.png', 'assets/hero', 'png'])
        const owners: ResolveOwners = [{ manifestId: seedManifest(items), fuzzy: true }]

        expect(resolveNamesLocally(owners, ['bulk-2000'], 4)!.resolved).toEqual({
            'bulk-2000': 'assets/bulk-2000',
        })
        expect(resolveNamesLocally(owners, ['hero'], 0)!.resolved).toEqual({
            hero: 'assets/hero',
        })
        expect(resolveNamesLocally(owners, ['bulk-near-miss'], 4)!.resolved).toEqual({})
    })
})
