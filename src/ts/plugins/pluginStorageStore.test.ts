...
describe('preloadAll + sync ops (V2 mode)', () => {
    ...
    test('releases the unbounded sync cache when legacy mode is disabled', async () => {
        for (let i = 0; i < 20; i++) seed('k' + i, 'v'.repeat(50))
        store.setCacheCap(0)
        await store.preloadAll()
        expect(store.getItemSync('k0')).toBe('v'.repeat(50))

        store.releasePreload()
        expect(store.isPreloaded()).toBe(false)
        expect(store.getItemSync('k0')).toBeNull()
        expect(await store.getItem('k0')).toBe('v'.repeat(50))
