...
// Leaving legacy sync mode must restore the bounded LRU. Without this, merely
// enabling a V2 plugin once pins every plugin value in browser memory forever
// and every later index refresh downloads newly discovered keys even after the
// legacy plugin is disabled.
export function releasePreload(): void {
    preloadGeneration++;
    preloaded = false;
    preloadPromise = null;
    evict();
