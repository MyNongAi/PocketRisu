...
function normalizedCharacter() {
    return {
        chats: [{ message: [], id: 'chat-1', localLore: [], fmIndex: -1 }],
        chatPage: 0,
        type: 'character',
        chaId: 'char-1',
        sdData: {},
...
describe('needsCharacterRuntimeNormalization', () => {
    it('skips cards that already satisfy runtime invariants', () => {
    ...
    it('detects legacy lorebooks and incomplete chat defaults', () => {
        ...
        expect(needsCharacterRuntimeNormalization({
            ...normalizedCharacter(),
            chats: [{ message: [], id: '', localLore: [], fmIndex: -1 }],
        })).toBe(true)
    })

    it('detects pending post-history instructions', () => {
        expect(needsCharacterRuntimeNormalization({
            ...normalizedCharacter(),
