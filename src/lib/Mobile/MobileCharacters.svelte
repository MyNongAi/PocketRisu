...
<VirtualList items={sortedCharacters} itemHeight={76} className="w-full h-full" key={(char) => char.chaId}>
    {#snippet children(char, i)}
        <div class="flex h-full items-center border-t-darkborderc w-full" class:border-t={i !== 0}>
            ...
            <button class="rounded-md p-2 text-textcolor2 transition-colors hover:bg-selected hover:text-textcolor" title="제목 색변경" aria-label="제목 색변경" onclick={() => editCharacterTitleColor(char.chaId)}>
                <PaletteIcon size={20} />
            </button>
            {#if gridMode}
