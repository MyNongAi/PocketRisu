            ...
            {#if visible.length > 0}
                {#if hasHeader}
                    ...
                    <div class="flex items-center gap-2 w-full rounded-md px-2 py-2 mt-1 border-t border-darkborderc text-textcolor cursor-pointer hover:bg-selected/30 select-none"
                        ...
                        <span class="grow text-left truncate {group.folder ? '' : 'text-textcolor2'}" style:color={listTitleColor(group.folder?.titleColor, visible.some(hasMissingAssets))}>{#if visible.some(hasMissingAssets)}<span aria-label="에셋 누락" title="에셋 누락">❗</span> {/if}{group.folder?.favorite ? '★ ' : ''}{group.folder?.name ?? language.folderUncategorized}</span>
                        {#if !alertMode}
                            <button
