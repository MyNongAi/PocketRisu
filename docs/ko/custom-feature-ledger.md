...
- 상류 기준: 공식 PocketRisu v1.10.0
- 개조 브랜치: `feature/external-assets`
- 외부 프로젝트 직접 코드 병합: 없음
- 참고 관계: 5379 dev, HaejeokRisu, Danso 패치는 구조와 UX 아이디어만 비교했으며 코드를 복사하지 않았습니다.

## 기능군 장부

| 기능군 | 상태 | 대표 커밋 | 주요 소유 영역 | 상류 반영 전략 |
|---|---|---|---|---|
| 외부 에셋 URI·공급자·LRU | 완료, 운영 이전 대기 | `75b91e62`, `e542b1ec` | `src/ts/storage`, `server/node`, 시스템 설정 | URI·백업 정책 RFC 후 분할 PR |
...
- 약 47GB 기존 에셋의 C → H 실제 복사·해시 검증·참조 전환
