...
function computeKeyPassword(password, salt) {
    ...
    const bcryptSalt = '$2y$10$' + bcrypt.encodeBase64(saltBytes, saltBytes.length);
    const hashed = bcrypt.hashSync(password, bcryptSalt);
    return hashed.slice(7);
}
...
function hashPasswordForSRP(keyPassword, salt) {
    const saltBytes = Buffer.from(salt, 'base64');
    const bcryptSalt = '$2y$10$' + bcrypt.encodeBase64(saltBytes, saltBytes.length);
    const bcryptHash = bcrypt.hashSync(keyPassword, bcryptSalt);
...
async function srpAuthenticate(token, urlPassword) {
    ...
    const urlPasswordSalt = info.UrlPasswordSalt || srpInfo.UrlPasswordSalt;
    const srpSalt = srpInfo.Salt || urlPasswordSalt;

    if (!urlPasswordSalt) throw new Error('UrlPasswordSalt not found in API response');
    if (!modulus) throw new Error('Modulus not found in API response');
    if (!serverEphemeral) throw new Error('ServerEphemeral not found in API response');

    const modulusBytes = await extractModulus(modulus);
    const keyPassword = computeKeyPassword(urlPassword, urlPasswordSalt);
    const hashedPassword = hashPasswordForSRP(keyPassword, srpSalt);
